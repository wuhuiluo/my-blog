    ...
    <div class="article-list-wrapper">
      <ul v-if="articles.length > 0">
        <li class="article-item" v-for="article in articles" :key="article.id">
          ...
          <img class="article-image" :src="article.cover" v-if="article.cover" />
        </li>
      </ul>
      <div v-if="isLoadMore" @click="$emit('loadMore')" class="load-more"></div>
      <empty v-if="!articles.length"></empty>
    </div>
  </div>
</template>
